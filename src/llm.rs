...
/// Accumulated cost (micro-dollars). Sum of total_cost_usd from claude responses.
...
///
/// Applies the following by default (all confirmed via `claude --help`):
...
/// - `--no-session-persistence` : no session file created (avoids contention during parallel runs)
/// - `--output-format json`     : collects result / structured_output / total_cost_usd
///
/// `--bare` is not used. It skips OAuth/keychain, which breaks auth for subscription-login users.
#[derive(Clone, Debug)]
pub struct Llm {
    pub bin: String,
    ...
    /// If true, omit --safe-mode and load CLAUDE.md etc. from the execution directory
...
impl Llm {
    ...
    fn call_once(
        ...
        // Must write stdin and read stdout/stderr concurrently to avoid a deadlock from pipe buffer saturation.
        ...
        if v.get("is_error").and_then(|b| b.as_bool()).unwrap_or(false) {
            return Err(anyhow!(
                "claude returned an error response (subtype={}): {}",
    ...
    /// Structured generation enforcing a JSON Schema. Schema validation is done by the claude CLI,
    /// and the result comes back in the response's structured_output field. If absent, tries to extract from the body.
    pub fn json(
        &self,
        prompt: &str,
        system: Option<&str>,
